...
namespace pet_spa_system1.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Edit_Products(int id)
        {
            ...
            var categories = await _productService.GetAllProductCategoriesAsync();
            var suggestedProducts = await _productService.GetAllProductsAsync(1, 4);
            var viewModel = new ProductViewModel
            {
                Product = product,
                SuggestedProducts = suggestedProducts.Where(p => p.ProductId != id).ToList(),
                Categories = categories
            };
            return View(viewModel);
        }
        ...
        {
            await _petService.DisablePetAsync(id);
