...
namespace pet_spa_system1.Controllers
{
    public class BlogsController : Controller
    {
        ...
        public async Task<IActionResult> Detail(int id)
        {
            ...
            var model = await _blogService.GetBlogDetailAsync(id, currentUser?.UserId);
        ...
        public async Task<IActionResult> Create(BlogCreateViewModel model)
        {
            ...
            try
            {
                var blogId = await _blogService.CreateBlogAsync(model, currentUser.UserId);
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            var userRole = GetUserRole(currentUser.RoleId);
            if (!_blogService.CanUserEditBlog(id, currentUser.UserId, userRole))
            {
                return Forbid();
            }
            ...
            var model = new BlogCreateViewModel
            {
                ...
                AvailableCategories = await _blogService.GetCategoriesAsync()
            };
        ...
        public async Task<IActionResult> Edit(int id, BlogCreateViewModel model)
        {
            ...
            try
            {
                var success = await _blogService.UpdateBlogAsync(id, model, currentUser.UserId);
                if (!success)
                {
                    return NotFound();
                }

                TempData["SuccessMessage"] = "Blog đã được cập nhật thành công.";
                return RedirectToAction("Detail", new { id });
            }
            catch (Exception ex)
            {
                ModelState.AddModelError("", "Có lỗi xảy ra khi cập nhật blog: " + ex.Message);
                model.AvailableCategories = await _blogService.GetCategoriesAsync();
                return View("Create", model);
            }
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            {
                var userRole = GetUserRole(currentUser.RoleId);
                var success = await _blogService.DeleteBlogAsync(id, currentUser.UserId);
        ...
        public async Task<IActionResult> AddComment(int blogId, string content, int? parentCommentId = null)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"User logged in: {currentUser.Username}");
            ...
            try
            {
                var success = await _blogService.AddCommentAsync(blogId, content.Trim(), currentUser.UserId, parentCommentId);
                ...
                if (success)
                {
                    return Json(new {
                        ...
                        userRole = GetUserRole(currentUser.RoleId),
        ...
        public async Task<IActionResult> ToggleLike(int blogId)
        {
            ...
            try
            {
                var isLiked = await _blogService.ToggleLikeAsync(blogId, currentUser.UserId);
        ...
        public async Task<IActionResult> MyBlogs()
        {
            ...
            var blogs = await _blogService.GetBlogsByUserAsync(currentUser.UserId);
            return View(blogs);
        }
